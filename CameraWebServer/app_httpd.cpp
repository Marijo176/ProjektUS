...
static esp_err_t capture_handler(httpd_req_t *req) {
  ...
  if (fb->format != PIXFORMAT_GRAYSCALE) {
    log_e("Camera is not in grayscale mode");
    esp_camera_fb_return(fb);
    httpd_resp_send_500(req);
    return ESP_FAIL;
  }
  ...
  uint8_t *edge_buf = (uint8_t *)malloc(w * h);
  if (!edge_buf) {
    log_e("Not enough memory for edge buffer");
    esp_camera_fb_return(fb);
    httpd_resp_send_500(req);
    return ESP_FAIL;
  }
  ...
  bool ok = fmt2jpg(edge_buf, w * h, w, h, PIXFORMAT_GRAYSCALE, 80, &jpg_buf, &jpg_len);
  ...
  httpd_resp_set_type(req, "image/jpeg");
  httpd_resp_set_hdr(req, "Content-Disposition", "inline; filename=edge.jpg");
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
...
//   camera_fb_t *fb = NULL; //ZAKOMENTIRANO
...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  while (true) {
    ...
    uint32_t avg_frame_time = ra_filter_run(&ra_filter, frame_time);
    log_i(
      "EDGE MJPG: %uB %ums (%.1ffps), AVG: %ums (%.1ffps)",
